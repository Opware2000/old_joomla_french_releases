<?php
/**
* @version $Id: admin.categories.php 10002 2008-02-08 10:56:57Z willebil $
* @package Joomla
...
function editCategory( $uid=0, $section='' ) {
	...
	if (!$sections && $type != 'other') {
		echo "<script> alert('Vous devez avoir au moins une section pour pouvoir céer un catégorie'); window.history.go(-1); </script>\n";
		exit();
	}
	...
	if ( $uid ) {
		...
		switch ( $row->section ) {
			...
			case 'com_contact_details':
				...
				$link 	= 'Tableau - Catégorie de contacts';
				break;
		...
		if ( $row->section > 0 ) {
			...
			for( $i = 0; $i < $count; $i++ ) {
				switch ( $menus[$i]->type ) {
					...
					$menus[$i]->type = 'Tableau - Catégorie de contenu';
					break;

					case 'content_blog_category':
					$menus[$i]->type = 'Blog - Catégorie de contenu';
					break;

					case 'content_archive_category':
					$menus[$i]->type = 'Blog - Archive de catégorie';
					break;
				}
			}
			$lists['links']	= 1;
	...
	if ($row->section == 'com_contact_details') {
		$types[] = mosHTML::makeOption( 'contact_category_table', 'Tableau - Catégorie de contacts' );
	} else
	if ($row->section == 'com_newsfeeds') {
		$types[] = mosHTML::makeOption( 'newsfeed_category_table', 'Tableau - Catégorie de flux RSS' );
	} else
	if ($row->section == 'com_weblinks') {
		$types[] = mosHTML::makeOption( 'weblink_category_table', 'Tableau - Catégorie de liens web' );
	} else {
		$types[] = mosHTML::makeOption( 'content_category', 'Tableau - Catégorie de contenu' );
		$types[] = mosHTML::makeOption( 'content_blog_category', 'Blog - Catégorie de contenu' );
		$types[] = mosHTML::makeOption( 'content_archive_category', 'Blog - Archive de Catégorie' );
	} // if
...
function removeCategories( $section, $cid ) {
	...
	if (count( $cid ) < 1) {
		echo "<script> alert('Sélectionner une catégorie à supprimer'); window.history.go(-1);</script>\n";
		exit;
	}
	...
	if (count( $err )) {
		$cids = implode( "\', \'", $err );
		$msg = 'Les catégories: '. $cids .' ne peuvent être supprimées tant qu\'elles contiennent des articles';
...
function moveCategorySave( $cid, $sectionOld ) {
	...
	if (!is_array( $cid ) || count( $cid ) < 1) {
		echo "<script> alert('Sélectionnez un élément à déplacer'); window.history.go(-1);</script>\n";
		exit;
	}
	...
	$msg = ( (count($cid) - 1) ? 'Catégories' : 'Catégorie' ) .' deplacée(s) vers '. $sectionNew->name;
...
function copyCategorySave( $cid, $sectionOld ) {
	...
	$msg = ( (count($cid) - 1) ? 'Catégories' : 'Catégorie' ) .' copiée(s) vers '. $sectionNew->name;
	mosRedirect( 'index2.php?option=com_categories&section='. $sectionOld .'&mosmsg='. $msg );
}
