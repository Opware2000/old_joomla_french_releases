...
class HTML_Media {
					...
					<td>
						...
						<tr>
							<td align="right" width="200" style="padding-right:10px;white-space:nowrap">
								Cr&eacute;er un r&eacute;pertoire
							</td>
							<td>
							...
							<tr>
								...
								<td class="buttonOut" width="10">
									<a href="javascript:dirup()">
										<img src="components/com_media/images/btnFolderUp.gif" width="15" height="15" border="0" alt="Dossier parent" />
									</a>
								</td>
								<td align="right">
									Upload Fichier <small>[ Max = <?php echo ini_get( 'post_max_size' );?> ]</small>
									&nbsp;&nbsp;&nbsp;&nbsp;
		...
		<tr>
			<td>
				<div align="center" style="font-size:small;font-weight:bold;color:#CC0000;font-family: Helvetica, sans-serif;">
					Probleme de configuration: &quot;<?php echo $BASE_DIR.$BASE_ROOT; ?>&quot; inexistant.
				</div>
			</td>
		</tr>
		</table>
	...
	function show_image($img, $file, $info, $size, $listdir) {
		...
		$overlib .= '</tr>';
		$overlib .= '</table>';
		$overlib .= '<br/> *Cliquer pour Agrandir*';
		$overlib .= '<br/> *Cliquer pour obtenir URL image*';
		?>
	...
	function show_dir( $path, $dir, $listdir ) {
		...
		$overlib .= '</tr>';
		$overlib .= '</table>';
		$overlib .= '<br/> *Cliquer pour Ouvrir*';
		?>
		<div style="float:left; padding: 5px">
	...
	function show_doc($doc, $size, $listdir, $icon) {
		...
		$overlib .= '<br/><br/> *Cliquer pour obtenir URL image*';
		?>
		<div style="float:left; padding: 5px">
	...
	function imageStyle($listdir) {
		...
		function deleteImage(file) {
			if(confirm("Supprimer fichier \""+file+"\"?"))
			return true;
		...
		function deleteFolder(folder, numFiles) {
			if(numFiles > 0) {
				alert("Il y a "+numFiles+" fichiers/dossiers dans \""+folder+"\".\n\nVeuillez supprimer tous les fichiers/dossiers dans \""+folder+"\" avant.");
				return false;
			}

			if(confirm("Supprimer répertoire \""+folder+"\"?"))
			return true;
