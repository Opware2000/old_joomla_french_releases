...
function showUsers( $option ) {
	...
	. "\n AND name != 'USERS'"
	;
	$types[] = mosHTML::makeOption( '0', '- Sélectionner Groupe -' );
	$database->setQuery( $query );
	$types = array_merge( $types, $database->loadObjectList() );
	$lists['type'] = mosHTML::selectList( $types, 'filter_type', 'class="inputbox" size="1" onchange="document.adminForm.submit( );"', 'value', 'text', "$filter_type" );

	// get list of Log Status for dropdown filter
	$logged[] = mosHTML::makeOption( 0, '- Sélectionner par Statut de connexion - ');
	$logged[] = mosHTML::makeOption( 1, 'Connecté');
...
function saveUser( $task ) {
	...
	if ($isNew) {
		...
		if ( $row->gid != $original->gid ) {		
			...
			if (( $user_group == 'super administrator' && $my->gid != 25) ) {
				...
				echo "<script> alert('Vous ne pouvez pas modifier ce groupe car vous n\'êtes pas Super Administrateur'); window.history.go(-1); </script>\n";
				exit();
			} else if ( $my->gid == 24 && $original->gid == 24 ) {
				// disallow change of super-Admin by non-super admin
				echo "<script> alert('vous ne pouvez pas modifier le groupe d\'un autre Administrateur car vous n\'êtes pas Super Administrateur'); window.history.go(-1); </script>\n";
				exit();
	...
	*/
	...
	switch ( $task ) {
		case 'apply':
			$msg = 'Modifications appliquées pour: '. $row->name;
			mosRedirect( 'index2.php?option=com_users&task=editA&hidemainmenu=1&id='. $row->id, $msg );
			break;

		case 'save':
		default:
			$msg = 'Modifications enregistrées pour: '. $row->name;
	...
	if ( !$msg && count( $cid ) ) {
		...
		foreach ($cid as $id) {
			...
			// cannot delete Super Admin where it is the only one that exists
				$msg = "Vous ne pouvez pas supprimer ce Super Administrateur as it is the only active Super Administrator for your site";
			} else {
				// delete user
				$obj->delete( $id );
				$msg = $obj->getError();
	...
	if ( count( $cid ) ) {
		...
		foreach ($cid as $id) {
			...
			if ( $this_group == 'super administrator' && $my->gid != 25 ) {
				$msg = "Un compte Super Administrateur ne peut pas être supprimé!";
 			} else if ( $id == $my->id ){
 				$msg = "Vous ne pouvez pas supprimer votre propre compte!";
 			} else if ( ( $this_group == 'administrator' ) && ( $my->gid == 24 ) ){
 				$msg = "Vous ne pouvez pas supprimer un autre `Administrateur` seul un `Super Administrateur` peut le faire";
			} else {
				$obj->load( $id );
				$count = 2;
				if ( $obj->gid == 25 ) {
...
function changeUserBlock( $cid=null, $block=1, $option ) {
	...
	if (count( $cid ) < 1) {
		echo "<script type=\"text/javascript\"> alert('Sélectionner un élément à $action'); window.history.go(-1);</script>\n";
		exit;
	}
...
function changeUserBlock( $cid=null, $block=1, $option ) {
	...
	if (count( $cid ) < 1) {
		$action = $block ? 'block' : 'unblock';
		echo "<script> alert('Sélectionner un élément à $action'); window.history.go(-1);</script>\n";
		exit;
	}
...
function logoutUser( $cid=null, $option, $task ) {
	...
	if ( is_array( $cid ) ) {
		if (count( $cid ) < 1) {
			mosRedirect( 'index2.php?option='. $option, 'Veuillez sélectionner un utilisateur' );
		}
