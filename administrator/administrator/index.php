...
if (isset( $_POST['submit'] )) {
	...
	if (@$my->id) {
		...
		if ( strcmp( $my->password, $pass ) || !$acl->acl_check( 'administration', 'login', 'users', $my->usertype ) ) {
			mosErrorAlert("Identifiant, mot de passe ou niveau d'accès incorrect. Veuillez recommencer.", "document.location.href='index.php'");
		}
		...
		mosErrorAlert("Identifiant ou mot de passe incorrect. Veuillez recommencer.", "document.location.href='index.php?mosmsg=Identifiant ou mot de passe incorrect. Veuillez recommencer'");
	}
} else {
	initGzip();
